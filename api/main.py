...
async def briefs(email:str, start:datetime.datetime, end: Union[datetime.datetime, None] = None, 
                 certainty: Union[float,None] = None, use_hyde: bool = False) -> ApiResponse[BriefResponse]:
    ...
    response =  support.create_briefings_for(email = email, start_time=start, end_time= end, certainty = certainty, use_hyde = use_hyde)
    return ApiResponse.create(response)
