...
class PromptManager(PromptRetriever):
    ...
    def _name_to_id(self, name: str) -> str:
        ...
        if prompt_id is None or self._cache_expired():
            self._cache_prompt_keys()
    ...
    def get_prompt(self, name: str) -> dict[str, str]:
        prompt_id: str = self._name_to_id(name)
        result: dict[str, str] = self._get_response("get-prompt", prompt_id)
    ...
    def _cache_prompt_keys(self) -> None:
        prompts: dict[str, any] = self._get_prompts()
    ...
    def _get_prompts(self) -> dict[str, any]:
        result = self._get_response("get-prompts")
        return result

    @d.deep_freeze_args
    @cached(cache=TTLCache(maxsize=1024, ttl=86400))
    def _get_response(self, request: str, prompt_name: str = None, params: dict[str, any] = {}) -> dict[str, any]:
